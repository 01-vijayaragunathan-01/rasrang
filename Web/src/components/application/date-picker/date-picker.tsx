...
export const DatePicker = ({ value: valueProp, defaultValue, onChange, onApply, onCancel, size = "sm", ...props }: DatePickerProps) => {
    ...
    return (
        <AriaDatePicker aria-label="Date picker" shouldCloseOnSelect={false} {...props} value={value} onChange={setValue}>
            ...
            >
                <AriaDialog aria-label="Date picker" className="rounded-2xl bg-[#0F0F1A]/90 backdrop-blur-3xl border border-white/10 shadow-2xl outline-hidden overflow-hidden">
                    {({ close }) => (
                        <>
                            <div className="flex px-6 py-5">
                                <Calendar highlightedDates={highlightedDates} />
                            </div>
                            <div className="grid grid-cols-2 gap-3 border-t border-white/5 p-4 bg-white/5">
                                <Button
                                    size="md"
                                    color="secondary"
                                ...
                                <Button
                                    size="md"
                                    color="primary"
                                    className="bg-gradient-to-r from-[#22D3EE] to-[#9D01E9] text-white shadow-[0_0_20px_rgba(34,211,238,0.3)] border-none"
                                    onClick={() => {
                                        onApply?.();
                                        close();
                                    }}
