...
function createTextTexture(gl, text, font = 'bold 30px monospace', color = '#E4BD8D') {
...
class Title {
  constructor({ gl, plane, renderer, text, textColor = '#E4BD8D', font = '30px sans-serif' }) {
    autoBind(this);
    this.gl       = gl;
    this.plane    = plane;
    ...
    this.program  = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          // ── Warm colour grade: boost reds/oranges slightly, lift blacks ──
          color.rgb = pow(color.rgb, vec3(0.92));           // slight gamma lift
          color.r  += 0.04;                                  // warm red push
          color.g  += 0.015;                                 // subtle green
          color.rgb = mix(color.rgb, color.rgb * vec3(1.08, 0.97, 0.9), 0.35); // warm cast
...
class App {
  constructor(container, { items, bend, textColor = '#E4BD8D', borderRadius = 0, font = 'bold 30px Cinzel, serif', scrollSpeed = 2, scrollEase = 0.05 } = {}) {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    // Default items use colour images (no ?grayscale)
    const defaultItems = [
    ...
    this.mediasImages     = galleryItems.concat(galleryItems); // duplicate for infinite loop
